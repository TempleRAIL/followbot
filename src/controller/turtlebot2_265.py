...
class Controller:
    ...
    def find_goal2(self,currentp,current_vel,finalp,final_vel,avg_vel,dist_min):
        ...
        if xf^2+yf^2 >= dist_min:
            x_params = cubic_poly_motion_planning(x0,vx0,xf,vxf)
            y_params = cubic_poly_motion_planning(y0,vy0,yf,vyf)
            expected_time = convolve_and_sum(x_params,y_params,dist_min)
        ...
        v, w = pure_persuit(goal_pose) # to be changed with proportional control
